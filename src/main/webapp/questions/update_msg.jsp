    ...
    <UL>
        ...
        <c:when test="${passwd_cnt == 1 }"> <!-- 패스워드 일치 -->
          <c:choose>
            <c:when test="${cnt == 1}"> <!-- 글 수정 성공 -->
              <LI class='li_none'>
                <span class='span_success'>질문을 수정했습니다.</span>
              </LI>
            </c:when>
            <c:otherwise>    <!-- 글 수정 실패 -->
              <LI class='li_none'>
                <span class='span_fail'>질문 수정에 실패했습니다.</span>
              </LI>
              <LI class='li_none'>
      ...
      <c:choose>
        <c:when test="${cnt == 1 && passwd_cnt == 1}">
          <LI class='li_none'>
            <button type='button' 
                        onclick="location.href='./read.do?ques_no=${param.ques_no}&ques_no=${param.ques_no}'"
