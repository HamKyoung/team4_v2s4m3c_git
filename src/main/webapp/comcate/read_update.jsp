...
<jsp:include page="/menu/top.jsp" />
  ...
  <DIV class='title_line'><A href="../comintro/list_all.do">회사 직무별 목록</A>> ${comcateVO.cate_name} > 조회(수정)</DIV>
  ...
  <DIV id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_update' id='frm_update' method='POST' action='./update.do'>
      ...
      <label>회사 번호</label>
      <input type='number' name='comno' value='${comcateVO.comno}' required="required" 
                min="1" max="99999" step="1" style='width: 5%;'>
        
      <label>직무 이름</label>
      <input type='text' name='cate_name' value="${comcateVO.cate_name}" required="required" style='width: 25%;'>
 
      <label>순서</label>
      <input type='number' name='cate_seqno' value="${comcateVO.cate_seqno}" required="required" 
                min='1' max='1000' step='1' style='width: 5%;'>
  
      <label>형식</label>
      <select name='cate_visible'>
        <option value='Y' ${comcateVO.cate_visible == 'Y' ? "selected='selected'":"" }>Y</option>
        <option value='N' ${comcateVO.cate_visible == 'N' ? "selected='selected'":""}>N</option>
      </select>
