      ...
      <UL>
        <c:choose>
          <c:when test="${passwd_cnt == 1 }">
            ...
            <c:choose>
              <c:when test="${cnt == 1}">
                <!-- 글 수정 성공 -->
                <LI class='li_none'><span class='span_success'>회원 정보를
                    수정했습니다.</span></LI>
              </c:when>
              <c:otherwise>
                <!-- 글 수정 실패 -->
                <LI class='li_none'><span class='span_fail'>회원 정보를
                    수정에 실패했습니다.</span></LI>
        ...
        <c:choose>
          <c:when test="${cnt == 1 && passwd_cnt == 1}">
            <LI class='li_none'>
              <button type='button'
                onclick="location.href='./read.do?gen_memberno=${param.gen_memberno}'"
